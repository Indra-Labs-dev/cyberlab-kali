...
async function loadAssets() {
  ...
  try {
    ...
    if (criticalityFilter.value) params.set("criticality", criticalityFilter.value);
    const query = params.toString() ? `?${params.toString()}` : "";
    assets.value = await apiFetch<Asset[]>(`/api/assets${query}`);
  } finally {
    loading.value = false;
  }
}
...
<template>
  <div>
    ...
    <div class="px-8 py-6">
      ...
      <p v-else-if="assets.length === 0" class="text-sm text-slate-600">
        No assets yet. Click "+ New Asset" to register one.
      </p>
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-slate-800">
            <tr
              ...
              @click="router.push(`/targets/${asset.id}`)"
            >
              ...
              <td class="px-4 py-2">
                <span class="rounded px-1.5 py-0.5 text-xs" :class="criticalityColor[asset.criticality]">
                  {{ asset.criticality }}
                </span>
              </td>
              <td class="px-4 py-2 text-slate-500">{{ projectName(asset.project_id) }}</td>
              <td class="px-4 py-2">
                <span class="rounded px-1.5 py-0.5 text-xs" :class="authColor[asset.authorization_status]">
