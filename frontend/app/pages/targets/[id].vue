...
async function loadAll() {
  loading.value = true;
  try {
    target.value = await apiFetch<Target>(`/api/targets/${targetId}`);
    const [proj, jbs, fnds, allTools] = await Promise.all([
      apiFetch<Project>(`/api/projects/${target.value.project_id}`),
      apiFetch<Job[]>(`/api/jobs?target_id=${targetId}&limit=100`),
      apiFetch<Finding[]>(`/api/findings?target_id=${targetId}&limit=200`),
...
function selectToolForScan(toolName: string) {
  ...
  tool?.arguments.forEach((arg) => {
    if (arg.type === "target" || arg.type === "url") return; // fixed to this target
...
async function runScan() {
  ...
  try {
    const job = await apiFetch<{ id: string }>("/api/jobs", {
      method: "POST",
      body: { tool: selectedTool.value, target_id: targetId, options },
    });
...
<template>
  <div>
    <PageHeader :title="target?.name || 'Target'" :subtitle="target ? address(target) : ''" />

    <div v-if="loading" class="px-8 py-6 text-sm text-slate-600">Loading…</div>

    <div v-else-if="target" class="space-y-6 px-8 py-6">
      <div class="flex flex-wrap items-center gap-3">
        <span class="rounded px-2 py-0.5 text-xs" :class="authColor[target.authorization_status]">
          {{ target.authorization_status }}
        </span>
        <span class="text-xs text-slate-500">{{ target.target_type }}</span>
        <NuxtLink v-if="project" :to="`/projects/${project.id}`" class="text-xs text-emerald-400 hover:underline">
          {{ project.name }}
        </NuxtLink>
        <NuxtLink :to="`/ai?target_id=${targetId}`" class="ml-auto text-xs text-emerald-400 hover:underline">
          Ask AI about this target
        </NuxtLink>
        <button
      ...
      <div v-if="target.authorization_status === 'UNKNOWN'" class="rounded-lg border border-amber-900 bg-amber-950/20 p-3 text-sm text-amber-400">
        This target's authorization status is UNKNOWN — jobs will be rejected until you mark it LAB, AUTHORIZED, or LOCAL.
      </div>
      ...
      <div class="rounded-lg border border-slate-800 bg-slate-900/40 p-4">
        ...
        <div class="flex flex-wrap gap-2">
          <button
            v-for="status in ['LAB', 'AUTHORIZED', 'LOCAL', 'UNKNOWN']"
            :key="status"
        ...
        <p v-if="target.description" class="mt-3 text-sm text-slate-400">{{ target.description }}</p>
      </div>
      ...
      <div class="rounded-lg border border-slate-800 bg-slate-900/40 p-4">
        ...
        <p v-if="jobs.length === 0" class="text-sm text-slate-600">No scans against this target yet.</p>
      ...
      <div class="rounded-lg border border-slate-800 bg-slate-900/40 p-4">
        ...
        <p v-if="findings.length === 0" class="text-sm text-slate-600">No findings for this target yet.</p>
        <div v-else class="space-y-2">
          <NuxtLink
            v-for="f in findings"
            :key="f.id"
