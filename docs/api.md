...
## Findings (Phase 9, Risk Score Phase 15 — voir [findings-reports.md](findings-reports.md) et [phase-15-risk-score.md](phase-15-risk-score.md))

- `GET /api/findings?severity=&job_id=&project_id=&target_id=&priority=&kev=&min_risk_score=&sort=&limit=` — liste les findings. `sort` : `created_at_desc` (défaut) | `risk_score_desc` | `risk_score_asc`. `priority` filtre sur `risk_priority` (`INFORMATIONAL`/`LOW`/`MEDIUM`/`HIGH`/`CRITICAL`), `kev=true` sur les findings listés dans CISA KEV, `min_risk_score` (0–100).
- `GET /api/findings/{id}` — détail d'un finding (`404` si inconnu). Inclut désormais `cve_ids`/`cvss_score`/`epss_score`/`kev`/`risk_score`/`risk_priority`/`risk_calculated_at` (cache Risk Score, `null` tant que non calculé).
