...
Une consolidation ciblée post-Phase-23 (non numérotée, même esprit) a ensuite fermé deux findings de robustesse restants identifiés par un post-audit read-only indépendant : timeout explicite + fail-closed déterministe du rate limiter sous panne Redis (`app/core/rate_limit.py`), et détection/réparation conservatrice des Missions/ChainRuns bloqués par un échec de commit entre `queue.enqueue()` et la validation finale de la transaction (`app/jobs/reconciliation.py::reconcile_stuck_missions`/`reconcile_stuck_chain_runs`, ré-invoquant `advance_mission()`/`advance_chain_run()` existants plutôt qu'une mutation d'état ad hoc).

## 7ter. Phase 24 — Attack Path Analysis (léger)

Première piste de la section 8 ci-dessous lancée **avant** que son prérequis explicite (Security Graph alimenté par des données réelles pendant plusieurs mois) ne soit rempli — décision explicite de l'utilisateur, pas une réévaluation du prérequis lui-même, qui reste valable pour les 3 autres pistes de la section 8. Recherche de chemins (pas un voisinage, distinct de `local_graph()` Phase 17) dans `graph_edges` selon deux modes : vers tout Asset `CRITICAL`, ou entre deux nœuds explicitement choisis. Chaque chemin est présenté comme une hypothèse structurelle à vérifier manuellement, jamais une preuve d'exploitabilité — aucun score d'exploitabilité/probabilité, aucune donnée inventée. Voir [phase-24-attack-path-analysis.md](phase-24-attack-path-analysis.md) pour l'architecture complète et [security.md](security.md) pour l'audit de sécurité.

## 7quater. SOC-lite, Multi-Kali, Plugin System — override explicite des 3 gates restantes du §8

Les 3 pistes restantes du §8 ont ensuite été lancées par un second override explicite de l'utilisateur, malgré des prérequis d'usage réel toujours non remplis (vérifiés contre le code/les données réels avant implémentation, comme pour la Phase 24) — avec la même contrainte : le plus petit périmètre fonctionnel correspondant au texte exact de la roadmap, sans infrastructure d'entreprise, sans franchir les exclusions du §9.
...
**Plugin System** — livré, dans un périmètre volontairement réduit par rapport au texte original du §8 : aucun des cas d'usage cités (Burp, Wireshark) n'a jamais été intégré à CyberLab, et les construire sans besoin concret aurait produit exactement la « mauvaise abstraction » que la roadmap met elle-même en garde contre. Le mécanisme existant (Tool Registry YAML + allowlist Kali) est généralisé à un second répertoire de définitions, contrôlé par l'opérateur (`TOOL_DEFINITIONS_EXTRA_DIR` côté API, `EXTRA_ALLOWED_TOOLS` côté agent Kali), réutilisant intégralement le schéma, le Policy Engine et le pipeline d'exécution existants — toujours uniquement des binaires CLI invocables par sous-processus dans le conteneur Kali, jamais un nouveau transport vers un service externe. Voir [phase-plugin-system.md](phase-plugin-system.md).
...
- **Attack Path Analysis (léger)** — **livré en Phase 24**, avant que le prérequis ci-dessous ne soit rempli (décision explicite de l'utilisateur, voir §7ter) : requêtes de chemins (`WITH RECURSIVE` sur `graph_edges`) présentées explicitement comme des **hypothèses à vérifier**, jamais des faits — reprend la mise en garde du brouillon original (section 38) qui est juste, et l'approche mesurée de FalconForce/BloodHound sur MITRE ATT&CK. Le prérequis lui-même (Security Graph alimenté par de vraies données pendant plusieurs mois) reste valable et non rempli — la valeur démontrable de la fonctionnalité restera limitée tant que ce n'est pas le cas.
- **Mode SOC-lite** — **livré** (voir §7quater) : pas une plateforme SOC complète (le brouillon V3 section 24/41/42 décrit littéralement un SIEM), mais une vue "Findings actifs + changements récents", désormais réellement affichée sur le Dashboard plutôt que seulement "couverte par les données" — l'inspection préalable a montré que le Dashboard existant n'affichait en réalité ni l'un ni l'autre avant cette implémentation. Le module Incident/Investigation séparé reste non construit, gated par son propre "si un besoin concret apparaît", non additionnellement levé par cet override.
...
- **Plugin System** — **livré** (voir §7quater), dans un périmètre délibérément plus étroit que l'idée originale : pas d'API de plugin générique conçue pour des intégrations hypothétiques (Burp, Wireshark — toujours aucun cas d'usage réel), mais une généralisation minimale du mécanisme YAML + allowlist déjà existant à un second répertoire contrôlé par l'opérateur. L'intégration de services externes non-CLI (Burp, Wireshark) reste hors périmètre, faute de besoin concret démontré — voir [phase-plugin-system.md](phase-plugin-system.md).
